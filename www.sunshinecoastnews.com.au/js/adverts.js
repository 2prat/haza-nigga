(function (window, instanceName, className) {
  ...
  var FabUtils = function () {
    ...
    this.errors = {
      ...
      isString: function (value, name, method) {
        if (typeof value !== "string") this.throwError(name, method, "string");
      },
  ...
  var FabPlugin = function () {
    ...
    this.callDetected = function () {
      if (callbackDetected === null) return false;
      callbackDetected();
    ...
    this.callUndetected = function () {
      if (callbackUndetected === null) return false;
      callbackUndetected();
  ...
  var Fab = function () {
    FabUtils.apply(this);
    this.options.set({ timeout: 200 });
    var self = this;
    var version = [4, 0, 0, "beta", 3];
    var events = {};
    var pluginsClass = {};
    this.getVersion = function (toInt) {
      if (toInt !== true) return version;
      else this.versionToInt(version);
    };
    ...
    this.check = function (pluginsList, optionsList) {
      ...
      if (pluginsList === undefined) pluginsList = Object.keys(pluginsClass);
      ...
      var end = function (pluginName, detected, force) {
        ...
        if (
          ...
          for (var name in plugins) plugins[name].instance.stop();
      ...
      for (var name in plugins) plugins[name].instance.start();
  ...
  var FabPluginHtml = function () {
    ...
    this.start = function () {
      ...
      if (this.options.get("baitElement") === null) {
        ...
        if (baitParent === null) window.document.body.appendChild(data.bait);
        else baitParent.appendChild(data.bait);
      } else data.bait = this.options.get("baitElement");
      var check = function () {
        if (self.checkBait(data.bait, true) === true) self.callDetected();
      };
    ...
    this.stop = function () {
      ...
      var baitParent = this.options.get("baitParent");
      if (baitParent === null) window.document.body.removeChild(data.bait);
      else baitParent.removeChild(data.bait);
      return this;
    };
  ...
  var FabPluginHttp = function () {
    ...
    this.options.set({
      baitMode: "ajax",
      baitUrl:
        "/ad/banner/_adsense_/_adserver/_adview_.ad.json?adzone=top&adsize=300x250&advid={RANDOM}",
    });
    var data = {};
    this.start = function () {
      var self = this;
    ...
    this._urlCheck = function (url, mode, cbDetected, cbUndetected) {
      ...
      if (mode === "ajax") {
        ...
        var respond = function (responseForce) {
          if (responseForce !== undefined) end(responseForce);
          else {
            if (status === 0) {
              end(true);
              return;
        ...
        xmlHttp.onreadystatechange = function () {
          ...
          if (xmlHttp.readyState === 4) respond();
        };
        try {
          xmlHttp.open("GET", url, true);
          xmlHttp.send();
        } catch (e) {
          if (e.result == "2153644038") respond(true);
        }
      } else if (mode === "import") {
...
})(window, "bdda51a2976d", "afbb813dd197");
